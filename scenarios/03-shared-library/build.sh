#!/bin/bash
# Filename: scenarios/03-shared-library/build.sh

# Shared Library Build Script for uni-algo
# This script demonstrates how to build with g++/clang++/cl directly

set -e  # Exit on any error

echo "=== Shared Library uni-algo Build ==="

# Check if Conan has been run
if [ ! -f "conan_toolchain.cmake" ]; then
    echo "Running conan install..."
    conan install . -o "uni-algo/*:header_only=False" -o "uni-algo/*:shared=True" --build=missing -s compiler.cppstd=17
fi
...
echo ""
echo "=== Build Complete ==="
echo "Run the examples using ./run.sh script or set library path manually."
echo "For Windows: copy DLL files to current directory or use system PATH."
